...
Module.register("camera", {
    ...
    start: function() { 
        this.sendSocketNotification('DETECT_FACE', this.config);
    ...
    detectface: function(){
		...
		var interval = setInterval(function() {
			if (timer === 4) {
				...
				self.counter.innerHTML = "Initializing module and detecting face <br>"+timer;
				timer++;
			}
		}, 1000);
    ...
    dotask: function(){
		...
		var interval = setInterval(function() {
			if (timer === 0) {
                self.counter.innerHTML = "Done Detecting!";
				timer--;
			}
            if (timer === -3){
                clearInterval(interval);
			...
			if (timer > 0) {
				self.counter.innerHTML = "Face Detected! Stay still for "+timer + " seconds.";
				timer--;
			}
		}, 1000);
    ...
    endtask: function(){
      this.counter.innerHTML = "Done Detecting!";
      this.updateDom(500);  
    },
    ...
    notificationReceived: function(notification, payload, sender) {
		if (notification === "SHOW_CAMERA" && this.display === false){
            ...
            this.detectface();
            this.sendNotification('HIDE_NEWS', this.config);
            this.sendNotification('HIDE_VOICE_CONTROL', this.config);
            this.sendNotification('HIDE_CLOCK', this.config);
            this.updateDom(500);
		}

        if (notification === "HIDE_CAMERA" && this.display == true){
            this.sendNotification('SHOW_NEWS', this.config);
            this.sendNotification('SHOW_VOICE_CONTROL', this.config);
            this.sendNotification('SHOW_CLOCK', this.config);
			this.display = false;
			this.updateDom(500);
		}
        if (notification === "DETECT_FACE"){
            this.display = true;
            this.detectface();
